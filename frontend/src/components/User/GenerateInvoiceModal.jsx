          ...
          {invoiceType ? (
            <div className="p-2 sm:p-3 md:p-6 bg-gray-50">
              <div className="border border-gray-300 rounded-lg p-2 sm:p-3 md:p-6 bg-white shadow-lg max-w-4xl mx-auto" style={{ fontFamily: "'Times New Roman', Times, serif" }}>
                ...
                <div className="flex justify-between items-start mb-3 sm:mb-4 md:mb-6 pb-2 sm:pb-3 md:pb-4 border-b-2 border-gray-200">
                  <div>
                    {companySettings?.company_logo_url && (
                      <img src={companySettings.company_logo_url} alt="Company Logo" className="mb-1 sm:mb-2 object-contain" style={{ height: `${logoSize}px`, maxWidth: '200px' }} />
                    )}
                ...
                {/* Invoice Dates */}
                <div className="mb-2 sm:mb-3 md:mb-4 flex flex-col items-end space-y-1">
                  <p className="text-xs text-gray-600 text-right">Tanggal Invoice: {new Date(invoiceDate).toLocaleDateString('id-ID')}</p>
                  <p className="text-xs text-gray-600 text-right">Jatuh Tempo: {new Date(dueDate).toLocaleDateString('id-ID')}</p>
                  <p className="text-xs sm:text-sm text-gray-600 text-right">Tanggal Layanan: {new Date(booking.booking_date).toLocaleDateString('id-ID')}</p>
                </div>
        ...
        <div className="absolute" style={{ left: '-9999px', top: 0, zIndex: -1 }}>
          <div ref={invoiceRef} className="bg-white pt-4 pb-4 px-4" style={{ width: '210mm', maxHeight: '297mm', fontFamily: "'Times New Roman', Times, serif" }}>
            ...
            <div className="flex justify-between items-start mt-1 mb-1 pb-1 border-b-2 border-gray-300">
              <div>
                {companySettings?.company_logo_url && (
                  <img src={companySettings.company_logo_url} alt="Company Logo" className="object-contain" style={{ height: `${logoSize}px`, maxWidth: '200px' }} />
                )}
            ...
            <table className="w-full border-collapse" style={{ fontFamily: "'Times New Roman', Times, serif" }}>
                ...
                {amounts.services && amounts.services.length > 0 ? (
                      ...
                      <tr key={index} className="border-b border-gray-200">
                        <td className="py-2 px-3 text-xs align-top border-r border-gray-200">
                          <div className="font-semibold text-gray-900 break-words">
                            {service.service_name}
                            {service.quantity > 1 && <span className="ml-1 text-green-600 font-bold text-[9px]">(x{service.quantity})</span>}
                            {bookingDays > 1 && <span className="ml-1 text-blue-600 font-bold text-[9px]">x{bookingDays} hari</span>}
                          </div>
                        ...
                        <td className="py-2 px-3 text-xs font-semibold text-gray-900 text-right align-top">
                          ...
                          {(service.quantity > 1 || bookingDays > 1) && (
                            <div className="text-[9px] text-gray-500 mt-1 break-words">
                              ({service.quantity || 1} x Rp {Math.round(service.custom_price || 0).toLocaleString('id-ID')} x {bookingDays} hari)
                            </div>
                          )}
                        </td>
                  ...
                  <tr className="border-b border-gray-200">
                    <td className="py-2 px-3 text-xs align-top border-r border-gray-200">
                      <div className="font-semibold text-gray-900 break-words">
                        ...
                        {(booking.booking_details?.booking_days > 1) && (
                          <span className="ml-1 text-blue-600 font-bold text-[9px]">x{booking.booking_details.booking_days}</span>
                        )}
                      </div>
                    </td>
                    <td className="py-2 px-2 text-xs text-gray-600 text-center align-top border-r border-gray-200">
                    ...
                    <td className="py-2 px-3 text-xs font-semibold text-gray-900 text-right align-top">
                      ...
                      {bookingDays > 1 && <div className="text-[9px] text-gray-500 mt-1 break-words">(Rp {Math.round((booking.total_amount || 0) / bookingDays).toLocaleString('id-ID')} x {bookingDays} hari)</div>}
                    </td>
                  </tr>
                )}
              ...
              {/* Totals Section - Modern Box */}
              <div className="flex justify-end mt-4 print:break-before-page">
                <div className="w-48 sm:w-56 md:w-64 lg:w-72 bg-white/90 border border-gray-200 rounded-lg shadow-sm p-3 space-y-1">
                  {/* Services Subtotal */}
                  <div className="grid grid-cols-[1fr_auto] gap-6 text-xs">
                    <span className="text-gray-600">Subtotal Layanan:</span>
                    <span className="font-semibold">Rp {Math.round(amounts.servicesSubtotal).toLocaleString('id-ID')}</span>
                  </div>
                  ...
                  {/* Grand Total */}
                  <div className="grid grid-cols-[1fr_auto] gap-6 text-xs font-bold border-t border-gray-300 pt-2 mt-2">
                    <span className="text-gray-900">TOTAL KESELURUHAN:</span>
                    <span className="text-blue-600">Rp {Math.round(amounts.total).toLocaleString('id-ID')}</span>
                  </div>
                  ...
                  {invoiceType === 'dp' && (
                    <>
                      ...
                      <div className="grid grid-cols-[1fr_auto] gap-6 text-xs bg-orange-50/50 p-2 rounded">
                        <span className="text-gray-700 font-medium">DP yang harus dibayar:</span>
                        <span className="font-bold text-orange-600">Rp {Math.round(amounts.dp).toLocaleString('id-ID')}</span>
                      </div>
                      <div className="grid grid-cols-[1fr_auto] gap-6 text-xs">
            ...
            <div className="bg-gray-50 border border-gray-300 rounded p-1 mt-4 w-3/4 mx-auto print:break-before-page">
              <h3 className="text-xs font-bold text-gray-900 mb-0.5">Informasi Pembayaran</h3>
            ...
            <div className="text-center text-xs text-gray-500 border-t pt-1 mt-2 w-3/4 mx-auto">
              <p>Invoice - {companySettings?.company_name || 'Nama Perusahaan'}</p>
            </div>
          </div>
        </div>
      )}
